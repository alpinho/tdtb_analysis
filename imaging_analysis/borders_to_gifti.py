...
def convert_fslr_to_gifti(border_file, output_gii, num_vertices=32492):
...
fslr32k_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              'fslr32k_meshes')
borders_folder = os.path.join(fslr32k_folder, 'borders')
...
if __name__ == '__main__':
    for hemi in ['L', 'R']:
        # Define file paths
        bfile = os.path.join(borders_folder,
                             'fs_LR.32k.' + hemi + '.border')
        gifti_file = os.path.join(borders_folder,
                                  'fs_LR.32k.' + hemi + '.border.label.gii')

        # Convert
        convert_fslr_to_gifti(bfile, gifti_file)
